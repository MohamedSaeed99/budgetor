...
def create_purchase(purchase: PurchaseEntity) -> PurchaseEntity:
    query = """
...
def update_purchase(purchase: PurchaseEntity) -> PurchaseEntity:
    query = """
        UPDATE purchases 
        SET tab_id = %s, purchase_date = %s, store = %s, amount = %s, category = %s, updated_at = CURRENT_TIMESTAMP
        WHERE id = %s AND user_id = %s
        RETURNING *
    """
    result = execute_query(
        query,
        (purchase.tab_id, purchase.purchase_date, purchase.store, purchase.amount, purchase.category, purchase.id, purchase.user_id),
        fetch_one=True
    )
